...
if [ "$IS_APPLE_SILICON" = true ]; then
    ...
    echo "   Using platform flag for Apple Silicon compatibility"
    docker buildx build --platform=linux/arm64 -t icyaml:platform-aware "$SCRIPT_DIR"
else
...
run_test() {
    ...
    if [ "$IS_APPLE_SILICON" = true ]; then
        # For Apple Silicon, use the properly built image with explicit platform
        docker run --platform=linux/arm64 --rm icyaml:platform-aware python3 --version
    else
        docker run --rm icyaml:platform-aware python3 --version
    fi
    ...
    if [ "$IS_APPLE_SILICON" = true ]; then
        docker run --platform=linux/arm64 --rm icyaml:platform-aware python3 -c "import yaml; print('YAML module is working!')"
    else
        docker run --rm icyaml:platform-aware python3 -c "import yaml; print('YAML module is working!')"
    fi
...
case "$command" in
    test)
        ...
        ;;
    outliers)
        run_docker_command "yaml_outlier_detector.py" "$@"
        ;;
    query)
        run_docker_command "yaml_query_engine.py" "$@"
        ;;
    scan)
        run_docker_command "yaml_tree_catalogger_enhanced.py" "$@"
        ;;
    shell)
